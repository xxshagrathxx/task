  ...
  <div class="{{ (!empty($containerNav) ? $containerNav : 'container-fluid') }} d-flex flex-wrap justify-content-between py-2 flex-md-row flex-column">
    <div class="mb-2 mb-md-0">
      @php
          ...
          $text = '';
          $lnkText = '';
          $lnk = $obj['copyrights_lnk'];

          if (LaravelLocalization::getCurrentLocale() == 'ar') {
            $text = $obj['copyrights_ar'];
            $lnkText = $obj['copyrights_ar_lnk'];
          } else {
            $text = $obj['copyrights_en'];
            $lnkText = $obj['copyrights_en_lnk'];
          }
      @endphp
      © {{ $text }}, <a href="{{ $lnk }}" target="_blank">{{ $lnkText }}</a>
      {{-- © <script>
